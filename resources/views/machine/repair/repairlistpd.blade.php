...
@section('contentandfooter')
    @php
    $months = [0 => 'ALL', 1 => 'มกราคม', 2 => 'กุมภาพันธ์', 3 => 'มีนาคม', 4 => 'เมษายน', 5 => 'พฤษภาคม', 6 => 'มิถุนายน', 7 => 'กรกฎาคม', 8 => 'สิงหาคม', 9 => 'กันยายน', 10 => 'ตุลาคม', 11 => 'พฤศจิกายน', 12 => 'ธันวาคม'];
                        ...
                        <div class="col-md-12">
                            <div class="card ">
                                <div class="card-header bg-primary  ">
                                    <form action="{{ route('pd.repairlist') }}" method="POST" id="FRM_SEARCH"
                                        ...
                                        <div class="row ">
                                            <div class="col-md-12 col-lg-12 form-inline my-1">
                                                ...
                                                <select class="form-control form-control-sm mt-1 mx-1" id="MONTH"
                                                    name="MONTH" onchange="changesubmit()">
                                                    <option value="0">ทั้งหมด</option>
                                                    @for ($m = 1; $m < 13; $m++)
                                                        <option value="{{ $m }}"
                                                            {{ $MONTH == $m ? 'selected' : '' }}>{{ $months[$m] }}
                                                        </option>
                                                    @endfor
                                                </select>
                                                ...
                                                <select class="form-control form-control-sm mt-1 mx-1" id="LINE" name='LINE'
                                                    ...
                                                    @foreach ($LINE as $index => $row_line)
                                                        <option value="{{ $row_line->LINE_CODE }}"
                                                            {{ $MACHINE_LINE == $row_line->LINE_CODE ? 'selected' : '' }}>
                                                            {{ $row_line->LINE_NAME }}</option>
                                                    @endforeach
                                                </select>
                                                <label class="text-white mx-2">เอกสาร : </label>
                                                <select class="form-control form-control-sm mt-1 mx-1" id="DOC_STATUS"
                                                    name="DOC_STATUS" onchange="changesubmit()">
                                                    <option value="0">ทั้งหมด</option>
                                                    <option value="9" {{ $DOC_STATUS == '9' ? 'selected' : '' }}>
                                                        ยังไม่ปิดเอกสาร</option>
                                                    <option value="1" {{ $DOC_STATUS == '1' ? 'selected' : '' }}>
                                                        ปิดเอกสารแล้ว</option>
                                                </select>
                                                <label class="text-white mx-1">ค้นหา : </label>
                                                <div class="input-group mx-1">
                                                    <input type="search" id="SEARCH_MACHINE" name="SEARCH_MACHINE"
                                                        class="form-control form-control-sm mt-1 col-lg-9"
                                                ...
                                                <div class="col-md-7 col-lg-1 text-right">
                                                    <a href="{{ route('repair.repairsearch') }}"
                                                        class="btn btn-warning  btn-xs mt-1 ">
                                ...
                                <div class="card-body">
                                        ...
                                        <div class="col-6 col-sm-6 col-md-3 col-lg-2 ml-auto my-1">
                                            <div class="selectgroup w-100">
                                                <label class="selectgroup-item">
                                                    <input type="radio" class="selectgroup-input" onchange="styletable(1)"
                                                        {{ Cookie::get('table_style_pd') == '1' ? 'checked' : (Cookie::get('table_style_pd') == '' ? 'checked' : '') }}
                                                        name="styletable">
                                                    <span class="selectgroup-button"><i class="fas fa-th-large"></i></span>
                                                </label>
                                                <label class="selectgroup-item">
                                                    <input type="radio" class="selectgroup-input" onchange="styletable(2)"
                                                        {{ Cookie::get('table_style_pd') == '2' ? 'checked' : '' }}
                                                        name="styletable">
                                    ...
                                    <div class="row" id="table_style"
                                        {{ Cookie::get('table_style_pd') == '1' ? '' : 'hidden' }}>
                                        @foreach ($dataset as $key => $row)
                                            ...
                                            <div class="col-lg-3">
                                                <div class="card card-round">
                                                    <div class="card-body">
                                                        <div class="card-title  fw-mediumbold {{ $BG_COLOR }}"
                                                            id="BG_{{ $row->UNID }}">
                                                            ...
                                                            <div class="item-list">
                                                                ...
                                                                <div class="info-user ml-3">
                                                                    <div class="username" style=""
                                                                        id="WORK_STATUS_{{ $row->UNID }}">
                                                                        {{ $WORK_STATUS }}</div>
                                                                    <div class="status">
                                                                        {{ $row->REPAIR_SUBSELECT_NAME }}</div>
                                                                    {!! $HTML_STATUS !!}
                                                                </div>
                                    ...
                                    <div class="table-responsive" id="list_table"
                                        {{ Cookie::get('table_style_pd') == '2' ? '' : 'hidden' }}>
                                                ...
                                                @foreach ($dataset as $key => $sub_row)
                                                    @php
                                                        ...
                                                        จัดเก็บเอกสารสำเร็จ
                                                            ...
                                                            $BTN =
                                                                '<button onclick=ConFirmForm(this) type="button"
                                                        data-unid="' .
                                                                $sub_row->UNID .
                                                                '"
                                                        data-docno="' .
                                                        ...
                                                        ดำเนินการสำเร็จ
                                                            ...
                                                            $BTN =
                                                                '<button class="btn btn-warning btn-sm btn-block my-1 text-left"
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td width="9%">{{ date('d-m-Y', strtotime($sub_row->DOC_DATE)) }}
                                                        </td>
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        function pdfsaverepair(unid) {
            var unid = unid;
            window.open('/machine/repair/savepdf/' + unid, 'RepairSaveprint',
                'width=1000,height=1000,resizable=yes,top=100,left=100,menubar=yes,toolbar=yes,scroll=yes');
