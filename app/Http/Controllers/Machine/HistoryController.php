...
class HistoryController extends Controller
{
  ...
  // public function PlanPDF(HistoryHeaderFooterPM $HistoryHeaderFooterPM,$MACHINE_UNID = NULL){
  //   $this->pdf = $HistoryHeaderFooterPM;
  //   $MACHINE_UNID =  0 ;
  //    $GROUP_HISTORY_PLAN = HistoryPlanPM::select('MACHINE_CODE','MACHINE_UNID','PM_PLAN_UNID','DOC_TYPE')
  //                                         ->where(function($query) use ($MACHINE_UNID){
  //                                           if ($MACHINE_UNID > 0) {
  //                                             $query->where('MACHINE_UNID','=',$MACHINE_UNID);
  //                                           }
  //                                         })->groupBy('MACHINE_UNID')->groupBy('MACHINE_CODE')
  //                                           ->groupBy('PM_PLAN_UNID')->groupBy('DOC_TYPE')
  //                                           ->orderBy('MACHINE_CODE')->get();
  //
  //   $DATA_PLAN_REUSLT = Pmplanresult::select('PM_MASTER_LIST_NAME','PM_MASTER_LIST_INDEX','PM_PLAN_UNID')
  //                                     ->groupBy('PM_MASTER_LIST_NAME')
  ...
  //   $this->pdf->SetFont('THSarabunNew','',14 );
  //
  //     foreach ($GROUP_HISTORY_PLAN as $groupindex => $grouprow) {
  //     $this->pdf->AliasNbPages();
  //     $this->pdf->AddPage(['L','A4',]);
  //     $this->pdf->header($grouprow->MACHINE_UNID);
  //     $this->pdf->setXY(5,45);
  ...
  //     $this->pdf->SetFont('THSarabunNew','',10);
  //
  //     $DATA_HISTORY_PLAN = HistoryPlanPM::select('*')->where('PM_PLAN_UNID','=',$grouprow->PM_PLAN_UNID)
  //                                         ->orderBy('DOC_DATE')->get();
  //     foreach ($DATA_HISTORY_PLAN as $index => $row) {
  //       $this->pdf->setX(5);
  //       $array_text_sparepart = array();
  //       $no_sparepart = 1;
  //       $no_woker = 1;
  //       foreach ($DATA_PLAN_REUSLT->where('PM_PLAN_UNID','=',$row->PM_PLAN_UNID) as $subindex => $row_sparepart) {
  //         $array_text_master[] = $no_sparepart++.'. '.$row_sparepart->PM_MASTER_LIST_NAME;
  //         $array_n[] =  "\n";
  //       }
  //       $TEXT = implode("\n",$array_text_master);
  //       $X= $this->pdf->getX();
  //       $this->pdf->SetWidths(array(10,10,12,20,20,20,20,20,40,48,24));
  //       $this->pdf->SetAligns(array('C','C','C','C','C','C','C','R','L','L','L'));
  //       $this->pdf->Row(array(
  //        $index+1
  //       ,$row->RANK
  //       ,iconv('UTF-8','cp874', $row->PERIOD.' เดือน')
  //       ,date('d-m-Y',strtotime($row->DOC_DATE))
  //       ,date('d-m-Y',strtotime($row->CHECK_DATE))
  //       ,iconv('UTF-8','cp874',date('H:i',strtotime($row->START_TIME)).' นาที')
  //       ,iconv('UTF-8','cp874',date('H:i',strtotime($row->END_TIME)).' นาที')
  //       ,iconv('UTF-8','cp874',number_format($row->DOWN_TIME).' นาที')
  //       ,iconv('UTF-8', 'cp874',$TEXT)
  //       ,iconv('UTF-8','cp874',($row->NOTE == "" ? '-' : $row->NOTE))
  //       ,iconv('UTF-8','cp874',$row->CHECK_BY)
  //       )) ;
  //
  //       $Y= $this->pdf->getY();
  //       if ($Y > 190) {
