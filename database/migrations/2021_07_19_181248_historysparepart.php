<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class Historysparepart extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
      Schema::create('PMCS_CMMS_HISTORY_SPAREPART', function (Blueprint $table) {
        $table->BigInteger('UNID')->primary();
        $table->BigInteger('SPAREPART_UNID')->nullable();
        $table->BigInteger('MACHINE_UNID')->nullable();
        $table->string('MACHINE_CODE',50)->nullable();
        $table->string('DOC_NO',50)->nullable();
        $table->date('DOC_DATE')->nullable();
        $table->integer('DOC_YEAR')->default(0);
        $table->integer('DOC_MONTH')->default(0);
        $table->integer('TOTAL')->default(0);
        $table->integer('IN_TOTAL')->default(0);
        $table->integer('OUT_TOTAL')->default(0);

        $table->BigInteger('UNID_REF')->nullable();
        $table->string('TYPE',50)->default(0);
        $table->string('RECODE_BY',200)->nullable();
        $table->string('REMARK',500)->nullable();

        $table->string('CREATE_BY',200)->nullable();
        $table->string('CREATE_TIME',50)->nullable();
        $table->string('MODIFY_BY',200)->nullable();
        $table->string('MODIFY_TIME',50)->nullable();

      });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('PMCS_CMMS_HISTORY_SPAREPART');
    }
}
